...
#define ETOD_LOG(x) cout << "[EToD Audio]  " << x << endl
	...
	AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			ETOD_LOG("File Info - " << filename << ":");
			ETOD_LOG("  Channels: " << channels);
			ETOD_LOG("  Sample Rate: " << sampleRate);
			ETOD_LOG("  Expected size: " << bufferSize);
		}
		...
		if (alGetError() != AL_NO_ERROR)
			ETOD_LOG("Failed to setup sound source");
	...
	AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			...
			ETOD_LOG("  Channels: " << channels);
			ETOD_LOG("  Sample Rate: " << sampleRate);
			ETOD_LOG("  Size: " << size << " bytes");

			auto [mins, secs] = result.GetLengthMinutesAndSeconds();
			ETOD_LOG("  Length: " << mins << "m" << secs << "s");
		}
	...
	static void PrintAudioDeviceInfo()
	{
		if (s_DebugLog)
		{
			ETOD_LOG("Audio Device Info:");
			ETOD_LOG("  Name: " << s_AudioDevice->DeviceName);
			ETOD_LOG("  Sample Rate: " << s_AudioDevice->Frequency);
			ETOD_LOG("  Max Sources: " << s_AudioDevice->SourcesMax);
			ETOD_LOG("    Mono: " << s_AudioDevice->NumMonoSources);
			ETOD_LOG("    Stereo: " << s_AudioDevice->NumStereoSources);
		}
	}
